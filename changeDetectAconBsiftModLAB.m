...
function  [xmin,ymin,wid,len] = changeDetectAconBsiftModLAB(pathA,pathB,figNum, iter, boxRatio, ransacThre,  labeledPositions, patch_w, win_size)
%     tic;
...
%     disp(['Max distance of annp channel-3: ',num2str(bnnp(bnnp3y_max,bnnp3x_max,3)) ]);
    disp(['PANO: ',pathA(strfind(pathA,'pano'):end),' FRAME: ',pathB(strfind(pathB,'image'):end), ...
        ' THRE: ',num2str(ransacThre),' BOXR: ',num2str(boxRatio)]);
...
%             [ymin1, ymax1, xmin1, xmax1, val1] = maxsubarray2D(unmatched);
%             unmatched(ymin1:ymax1,xmin1:xmax1) = 0;
%             if (ymax1 - ymin1 > 0 ) && (xmax1 - xmin1 > 0 ) && ymax1 <= size(unmatched,1) * 0.7 && ymax1 >= size(unmatched,1) * 0.1                  rectangle('position',[xmin1, ymin1, xmax1-xmin1, ymax1 - ymin1],'edgecolor',color(i),'LineWidth',2)
%                 xmin(i) = xmin1;
%                 ymin(i) = ymin1;
%                 wid(i) = xmax1-xmin1;
%                 len(i) = ymax1 - ymin1;
...
%     dir = strcat('./video/',videoName,'/LAB/Modulated/sift/withResizePatchMatch/Thre',num2str(ransacThre),'/AconsBiter',num2str(iter),'boxR',num2str(boxRatio));
