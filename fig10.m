...
function  [area,xmin,ymin,wid,len] = fig10(pathA,pathB,figNum, iter, boxRatio, ransacThre,  labeledPositions, patch_w, win_size,alphabet)
%     tic;
...
%     rrr = reshape(temp(1,:),[size(B,2),size(B,1)])';
    ...
    imagesc(A)    
    axis off
    print(strcat(dir,'/1_',pathB(strfind(pathB,'image'):end-4),'_', pathA(strfind(pathA,'pano')+4:end-4)),'-djpeg','-r500')
...
%     subplot(221) ,imagesc(maskDoubleBnnpCh3);
    ...
    hold off;
    axis off
    print(strcat(dir,'/2_',pathB(strfind(pathB,'image'):end-4),'_', pathA(strfind(pathA,'pano')+4:end-4)),'-djpeg','-r500')
%     print(strcat(dir,'/10',alphabet,'_2'),'-djpeg','-r500')
%     close 1
