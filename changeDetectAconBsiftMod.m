...
function  [score1,score2,score3] = changeDetectAconBsiftMod(pathA,pathB,videoName, figNum, iter, boxRatio, thre)
%     tic;
...
%     anotherFrameNum = mod((frameNum + 30), 100);
    ...
    if exist(C,'file') ~= 2
        ...
        if(anotherFrameNum < 9 )
            C = strcat(pathB(1:end-5),num2str(anotherFrameNum),'.jpg');
        elseif(anotherFrameNum < 99)
            C = strcat(pathB(1:end-6), num2str(anotherFrameNum),'.jpg') ;  
        else
            C = strcat(pathB(1:end-7), num2str(anotherFrameNum),'.jpg') ;  
...
%     imshow(annp(:,:,3),[]);
    ...
    unmatched = doubleBnnp .* mask - boxRatio * mean_bnnp3 ;% could set as a variable
    for i=1:3        
