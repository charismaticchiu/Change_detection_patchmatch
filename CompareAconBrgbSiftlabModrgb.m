...
function  [modArea ,baselineArea,xmin,ymin,wid,len] = CompareAconBrgbSiftlabModrgb(pathA,pathB,figNum, iter, boxRatio, ransacThre,  labeledPositions, patch_w, win_size)
...
%     mean_bnnp3 = mean(mean(error))
    ...
    disp(['labMod rgbPM PANO: ',pathA(strfind(pathA,'pano'):end),' FRAME: ',pathB(strfind(pathB,'image'):end), ...
...
%     tThre = boxRatio * mean_bnnp3;
    ...
    subplot(421) ,imagesc(maskDoubleBnnpCh3);
    title(['Modulated: ',pathB(strfind(pathB,'image'):end-4)]);
...
%         tempPrec = hit/usedPatchNum;
%         tempRecall = hit/numOfLabels;
%         if tempRecall 
...
%     min(min(unmatched))
    ...
    xlim([0 1.5])
    ylim([0 1.5])
    dir = strcat('./video/Label/rgbSiftlabModrgbPMCompare/ransacThre',...
