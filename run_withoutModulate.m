...
%             score{j}{k} = [];
            if  ~strcmp(images{j,k},'')% If there is a frame for this index k, then do find_ann0 
                for boxRatio = 0.5:0.1:2.0     
                    
                    [sc1,sc2,sc3]= changeDetectAconBwithoutMod(images{j,1},images{j,k} , i, j  *size(images,2) + k , iter,boxRatio);
%                     score{j}{k} = [score{j}{k};sc1,sc2,sc3];
                end
%                 figure;
%                 plot( score{j}{k} );
%                 dir = ['./video/',num2str(i),'/noModulated/withResizePatchMatch/'];
...
% ratio = matchedCoef2./matchedCoeqf;
